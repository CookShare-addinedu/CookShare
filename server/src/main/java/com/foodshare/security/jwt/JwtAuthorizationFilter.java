...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            String key = "BLACKLISTED_TOKEN:" + token; // 블랙리스트 토큰 검사를 위한 키 생성
            if (redisService.isTokenBlacklisted(key)) {
                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED); // 401 상태 코드 설정
                response.setContentType("application/json"); // 응답 컨텐트 타입 설정
                response.setCharacterEncoding("UTF-8"); // 문자 인코딩 설정
                response.getWriter().write("{\"error\":\"블랙리스트된 토큰입니다.\"}"); // JSON 형식의 오류 메시지 작성
                response.getWriter().flush(); // 버퍼를 비우고 데이터를 클라이언트로 전송
                // 필터 체인 실행 중지
        ...
        if (authorization != null && authorization.startsWith("Bearer ")) {
            authHeader = authorization.substring(7);
            try {
                if (!jwtUtil.isExpired(authHeader)) {
                    processAuthentication(authHeader, response);
                } else {
